...
var hero_atk: int; var hero_def: int
...
func _terrain_setup():
	...
	var cs = [Color(0.2, 0.35, 0.12, 0.3), Color(0.28, 0.22, 0.1, 0.3), Color(0.12, 0.2, 0.3, 0.3)]
...
func _floating_text(pos, txt, col):
	...
	lbl.size = Vector2(50, 16); lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	add_child(lbl)
	var t = create_tween(); t.set_parallel(true)
	t.tween_property(lbl, "position", pos + Vector2(0, -35), 0.7)
...
func _check_victory():
	...
	# Check if enemies reached player castle
	for u in enemy_units:
...
func _unhandled_input(event):
	if event is InputEventKey and event.pressed and not event.echo:
		match event.keycode:
			...
			KEY_Q: _hero_skill()
			KEY_SPACE: paused = !paused; status_label.text = "⏸ PAUSA" if paused else ("⚔️ BATALLA")
