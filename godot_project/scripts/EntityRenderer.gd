...
func _ready():
	...
	_hp_bg.mouse_filter = Control.MOUSE_FILTER_IGNORE; add_child(_hp_bg)
	
	_hp_fill = ColorRect.new()
	_hp_fill.size = Vector2(30, 4); _hp_fill.position = Vector2(-15, -28)
	_hp_fill.color = Color(0.2, 0.75, 0.2, 0.9)
	_hp_fill.mouse_filter = Control.MOUSE_FILTER_IGNORE; add_child(_hp_fill)
	
	# Load sprite texture (skip fallback if loaded)
	_load_texture()
...
func _load_texture():
	...
	key = "enemy_hero" if entity_type == "hero" and entity_team == "enemy" else ("hero" if entity_type == "hero" else key)
